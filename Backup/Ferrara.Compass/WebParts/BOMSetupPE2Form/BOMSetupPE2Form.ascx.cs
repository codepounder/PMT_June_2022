...
namespace Ferrara.Compass.WebParts.BOMSetupPE2Form
{
    [ToolboxItemAttribute(false)]
    public partial class BOMSetupPE2Form : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    LoggingService.LogError(LoggingService.WebPartLoggingDiagnosticArea, CompassForm.BOMSetupPE2.ToString() + ": " + exception.Message);
                    exceptionService.Handle(LogCategory.CriticalError, exception, CompassForm.BOMSetupPE2.ToString(), "Page_Load");
                }
            }
            else
            {
        ...
        private void InitializeScreen()
        {
            ...
            if (!userManagementService.HasReadAccess(CompassForm.BOMSetupPE2))
            {
        ...
        {
            hdnPageName.Value = GlobalConstants.PAGE_BOMSetupPE2;
            ...
            Utilities.SetDropDownValue(projectSummaryItem.AllAspectsApprovedFromPEPersp, this.ddlAllAspectsApprovedFromPEPersp, this.Page);
            txtWhatIsIncorrectPE.Text = projectSummaryItem.WhatIsIncorrectPE;
        ...
        private void LoadAttachment()
        {
            ...
            #endregion
            #region External Component Dieline
            var ExternalComponentDielines = BOMSetupService.GetUploadedFiles(ProjectNumber, GlobalConstants.DOCTYPE_ExternalComponentDieline);

            if (ExternalComponentDielines.Count > 0)
            {
                rptExternalComponentDieline.Visible = true;
                rptExternalComponentDieline.DataSource = ExternalComponentDielines;
                rptExternalComponentDieline.DataBind();
            }
            else
            {
                rptExternalComponentDieline.Visible = false;
            }
            #endregion
        }
        protected void lnkDeleteExternalComponentDieline_Click(object sender, EventArgs e)
        {
        ...
        {
            //exceptionService.Handle(LogCategory.General, "Looging timing for diagnostics", "PE2", "start: Load ucBOMGridPackMeas_New", DateTime.Now.ToString("yyyy MM dd HH:mm:ss FFF"));
            ...
            // exceptionService.Handle(LogCategory.General, "Looging timing for diagnostics", "PE2", "end: Load ucBOMGridPackMeas_New", DateTime.Now.ToString("yyyy MM dd HH:mm:ss FFF"));
        }
        ...
        private BillofMaterialsItem ConstructFormData(bool submitted)
        {
            ...
            try
            {
                ...
                bomItem.AllAspectsApprovedFromPEPersp = ddlAllAspectsApprovedFromPEPersp.SelectedItem.Text;
                bomItem.WhatIsIncorrectPE = txtWhatIsIncorrectPE.Text;
                if (submitted)
                {
        ...
        {
            notificationService.SendHelpDeskAccessEmail(SPContext.Current.Web.CurrentUser.LoginName, SPContext.Current.Web.CurrentUser.Email, "BOM Setup PE2");
            this.divAccessDenied.Visible = false;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            BOMSetupService.UpdateBillOfMaterialsItem(item, GlobalConstants.PAGE_BOMSetupPE2);

            ApprovalItem approvalItem = ConstructApprovalData();
            BOMSetupService.UpdateBillofMaterialsApprovalItem(approvalItem, GlobalConstants.PAGE_BOMSetupPE2, false);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hdnProjectType.Value == GlobalConstants.PROJECTTYPE_GraphicsChangeOnly && ddlAllAspectsApprovedFromPEPersp.SelectedItem.Text == "No")
                {
                    iserror = false;
                }
                else
                {
                    iserror = RequiredFieldCheckForPackagingComponent();
                }
                ...
                ApprovalItem approvalItem = ConstructApprovalData();
                BOMSetupService.UpdateBillofMaterialsApprovalItem(approvalItem, GlobalConstants.PAGE_BOMSetupPE2, true);
                // Complete the workflow task
                CompleteWorkflowTask();

                if (hdnProjectType.Value == GlobalConstants.PROJECTTYPE_GraphicsChangeOnly && ddlAllAspectsApprovedFromPEPersp.SelectedItem.Text == "No")
                {
                    var ProjectCancelReasson = txtWhatIsIncorrectPE.Text;
                    Utilities.UpdateProjectRejectionReason(iItemId, ProjectCancelReasson, GlobalConstants.PAGE_BOMSetupPE2);
