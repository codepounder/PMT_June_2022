...
namespace Ferrara.Compass.WebParts.MyWorkflowTasksForm3
{
    [ToolboxItemAttribute(false)]
    public partial class MyWorkflowTasksForm3 : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            timelineNumbers = dashboardService.GetWorkflowStepItems();
            ...
            List<ProjectTimelineItem> projectsItems = new List<ProjectTimelineItem>();
            if (newProjectNumbers.Count > 0)
            {
                lstDets = dashboardService.getAllProjectDetails(newProjectNumbers);
                projectsItems = dashboardService.GetProjectsItem(newProjectNumbers);
            }
            foreach (WorkflowTaskDetailsItem task in lstWSTasks)
            {
                try
            ...
            foreach (WorkflowTaskDetailsItem task in lst)
            {
                ...
                try
                {
                    ...
                    if (projectInfo != null)
                    {
                        ...
                        {
                            exceptionService.Handle(LogCategory.CriticalError, excep, "Workflow details tasks", "Line 198");
                        }
                        if (task.WorkflowStep != "SAPROUTINGSETUP_NOTIFICATION")
                        {
                            if (task.WorkflowStep == "Graphics")
                            {
                                task.WorkflowStep = "GRAPHICS";
                            }
                            else if (task.WorkflowStep.Contains("BOMSetupProc"))
                            {
                                task.WorkflowStep = "BOMSetupProc";
                            }
                            else if (task.WorkflowStep == "BOMSetupMaterialWarehouse")
                            {
                                task.WorkflowStep = "MaterialWarehouseSetUp";
                            }
                            else if (task.WorkflowStep == "FGPACKSPEC")
                            {
                                task.WorkflowStep = "FGPackSpec";
                            }
                            try
                            {

                                ProjectTimelineItem updateItem = (from item in projectsItems where item.CompassListItemId == projectInfo.CompassItemId select item).FirstOrDefault();

                                if (updateItem != null)
                                {
                                    updatedDay = Convert.ToInt32(updateItem.GetType().GetProperty(task.WorkflowStep).GetValue(updateItem));
                                }
                            }
                            catch (Exception exc)
                            {
                                exceptionService.Handle(LogCategory.CriticalError, exc, "Workflow details tasks", "Line 206" + task.WorkflowStep);
                            }
                            ...
                            if (updatedDay <= 0)
                            {
                                try
                                {
                                    if (task.WorkflowStep == "MaterialWarehouseSetUp")
                                    {
                                        task.WorkflowStep = "MatrlWHSetUp";
                                    }
                                    TimelineTypeItem timeLineItem = (from WFStep in timelineNumbers where WFStep.WorkflowQuickStep == task.WorkflowStep select WFStep).FirstOrDefault();
                                    task.DueDateCalc = Convert.ToInt32(timeLineItem.GetType().GetProperty(timelineType).GetValue(timeLineItem));
                                }
                                catch (Exception exce)
                                {
                                    exceptionService.Handle(LogCategory.CriticalError, exce, "Workflow details tasks", "task.WorkflowStep: " + task.WorkflowStep);
                                }
                            }
                            else
                            {
                ...
                catch (Exception ex)
                {
                    ...
                    exceptionService.Handle(LogCategory.CriticalError, ex, "Workflow details tasks", "Line 193");
                }
            }
            ...
            if (isProcurementMember && newProjectNumbers.Count > 0)
            {
                try
                {
                    ...
                    foreach (WorkflowTaskDetailsItem task in lst)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            exceptionService.Handle(LogCategory.CriticalError, ex, "Workflow details tasks", "Line 236");
                        }
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    exceptionService.Handle(LogCategory.CriticalError, ex, "Workflow details tasks", "Line230");
                }
            }
