...
namespace Ferrara.Compass.WebParts.EstPricingForm
{
    [ToolboxItemAttribute(false)]
    public partial class EstPricingForm : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    LoggingService.LogError(LoggingService.WebPartLoggingDiagnosticArea, CompassForm.EstPricing.ToString() + ": " + exception.Message);
                    exceptionService.Handle(LogCategory.CriticalError, exception, CompassForm.EstPricing.ToString(), "Page_Load");
                }
            }
            else
            {
        ...
        private void InitializeScreen()
        {
            ...
            if (!userMgmtService.HasReadAccess(CompassForm.EstPricing))
            {
            ...
            if (!userMgmtService.HasWriteAccess(CompassForm.EstPricing))
            {
        ...
        private void LoadFormData()
        {
            ...
            this.chkInitialEstimatedPricing.Checked = (item.InitialEstimatedPricing == "Yes" ? true : false);
        ...
        private TradePromoGroupItem ConstructFormData()
        {
            ...
            item.InitialEstimatedPricing = this.chkInitialEstimatedPricing.Checked ? "Yes" : "No";
            item.LastUpdatedFormName = CompassForm.EstPricing.ToString();
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tradePromoService.UpdateEstimatedPricingItem(item);

                var approvalItem = ConstructApprovalData();
                tradePromoService.UpdateEstimatedPricingApprovalItem(approvalItem, false);
            ...
            catch (Exception ex)
            {
                ...
                LoggingService.LogError(LoggingService.WebPartLoggingDiagnosticArea, CompassForm.EstPricing.ToString() + ": btnSave_Click: " + ex.Message);
                exceptionService.Handle(LogCategory.CriticalError, ex, CompassForm.EstPricing.ToString(), "btnSave_Click");
            }
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var approvalItem = ConstructApprovalData();
                tradePromoService.UpdateEstimatedPricingApprovalItem(approvalItem, true);

                // Complete the workflow task
                workflowService.CompleteWorkflowTask(iItemId, WorkflowStep.EstPricing);
            ...
            {
                ErrorSummary.AddError(ex.Message, this.Page);
                LoggingService.LogError(LoggingService.WebPartLoggingDiagnosticArea, CompassForm.EstPricing.ToString() + ": btnSubmit_Click: " + ex.Message);
                exceptionService.Handle(LogCategory.CriticalError, ex, CompassForm.EstPricing.ToString(), "btnSubmit_Click");
            }
        }
        #endregion
